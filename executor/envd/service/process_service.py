...
# Whitelist of allowed commands (base names only)
ALLOWED_COMMANDS: Set[str] = {
    # Shells
    "bash", "sh", "zsh", "fish",
    # Programming language interpreters
    "python", "python3", "node", "npm", "npx", "yarn", "pnpm",
    "ruby", "php", "perl", "lua", "java", "javac",
    # Build tools
    "make", "cmake", "gcc", "g++", "clang", "clang++", "go", "cargo", "rustc",
    # Version control
    "git", "svn", "hg",
    # Common utilities
    "ls", "cat", "echo", "pwd", "cd", "mkdir", "rm", "cp", "mv", "touch",
    ...
    # Others
    "claude", "anthropic",
    "uv", "pip", "pip3", "poetry", "pdm",
    "pytest", "pytest-watch", "tox", "nox",
    "black", "isort", "flake8", "pylint", "mypy", "ruff",
    "next", "tsc", "tsx", "eslint", "prettier",
}
...
class ProcessServiceHandler:
    ...
    def _validate_command(self, cmd: str, args: List[str]) -> None:
        ...
        Raises:
            ConnectError: If command is not allowed or arguments are invalid
        """
        import os

        # Check command length
        total_length = len(cmd) + sum(len(arg) for arg in args)
        if total_length > MAX_CMD_LENGTH:
            raise ConnectError(
                code="invalid_argument",
                message=f"Command exceeds maximum length of {MAX_CMD_LENGTH}",
            )
        ...
        if len(args) > MAX_ARG_COUNT:
            raise ConnectError(
                code="invalid_argument",
                message=f"Too many arguments (max {MAX_ARG_COUNT})",
            )
        ...
        for arg in args:
            if len(arg) > MAX_ARG_LENGTH:
                raise ConnectError(
                    code="invalid_argument",
                    message=f"Argument exceeds maximum length of {MAX_ARG_LENGTH}",
                )
        ...
        # Remove any extension for platform compatibility
        if IS_WINDOWS:
            cmd_base = os.path.splitext(cmd_base)[0].lower()
        else:
            cmd_base = cmd_base.lower()

        # Check if command is in whitelist
        if cmd_base not in ALLOWED_COMMANDS:
            # Allow full path commands if they exist in common bin directories
            if cmd.startswith("/"):
                allowed_prefixes = (
                    "/usr/bin/",
                    "/usr/local/bin/",
                ...
                if not any(cmd.startswith(prefix) for prefix in allowed_prefixes):
                    logger.warning(f"Command not in allowed paths: {cmd}")
                    raise ConnectError(
                        code="permission_denied",
                        message=f"Command not allowed: {cmd_base}",
                    )
            else:
                logger.warning(f"Command not in whitelist: {cmd_base}")
                raise ConnectError(
                    code="permission_denied",
                    message=f"Command not allowed: {cmd_base}",
                )

        # Additional validation: block dangerous characters in args
        dangerous_chars = set(";|&$`\n\r<>")
        for arg in args:
    ...
    async def Start(self, request: process_pb2.StartRequest):
        ...
        try:
            # Security validation: validate command and arguments
            self._validate_command(
                request.process.cmd, list(request.process.args)
            )
