...
`/new` 只会重置当前对话上下文，不会清除当前会话中已选择的智能体。
...
- `⭐ 当前`：表示当前会话实际生效的智能体
- `[默认]`：表示该 IM 通道配置的默认智能体

### 群聊中的智能体切换

在群聊中，智能体切换按“用户 + 会话”生效，而不是按整个群生效。

这意味着：

- 你在群里执行 `/agents 2` 后，只影响你自己在当前群会话中的后续消息
- 不会影响其他成员在同一个群里的智能体选择
- 机器人回复仍然会发送到群里公开可见

这样可以避免一个成员切换智能体后影响整个群的行为。
